...
int fy_parse_setup(struct fy_parser *fyp, const struct fy_parse_cfg *cfg)
{
	...
	if ((fyp->cfg.flags & (FYPCF_TAB_MASK << FYPCF_TAB_SHIFT)) == FYPCF_TAB_AUTO)
		fyp->tabsize = 0;	/* disabled by default, needs a marker to activate */
	...
	if (fyp->tabsize)
		fy_notice(fyp, "starting tab size set to %d", fyp->tabsize);
...
void fy_scan_tab_setting(struct fy_parser *fyp)
{
	...
	while (!(fy_is_breakz(c = fy_parse_peek_at(fyp, scan_ahead)))) {
...
ts_found:
	...
	fy_notice(fyp, "updating tab size to %d", n);
